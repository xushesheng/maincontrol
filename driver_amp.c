...
*******IP节点映射表*******
************************/
static u32 ip_to_nodeid(__be32 ip_be)
{
    ...
    /* 单播：192.168.1.10 ~ 192.168.1.25 -> 0 ~ 15 */
    if (b0 == 192 && b1 == 168 && b2 == 1) {
        if (b3 >= 10 && b3 <= 25)
            return (u32)(b3 - 10);  // 对应 node_id <= 15
    }

    return 0;
}
static u32 nodeid_to_ip(u32 node_id)
{
    u8 ip[4] = {192, 168, 1, 0};
...
static int process_udp_data(struct amp_net_msg *msg)
{
    ...
    //writel(msg->node_id, tx_node_id);
	uint32_t nodeid = ip_to_nodeid(msg->ip);
